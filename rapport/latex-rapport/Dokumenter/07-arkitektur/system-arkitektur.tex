...
Ved login bliver der sendes en \texttt{HTTP POST}-request fra klienten til autentifikation-endpointet med brugerens credentials.
Serveren validerer oplysninger ved at sammenligne det indtastede password med den gemte hashværdi (\texttt{bcrypt}).
...
Refresh-tokenet anvendes til at forny access tokens uden at brugeren skal logge ind igen.
Dette sker ved, at klienten sender refresh-tokenet til et dedikeret endpoint,
som ved succesfuld validering udsteder et nyt JWT med begrænset levetid.

Dette token som returneres til klienten, anvendes derefter til alle forespørgsler til \texttt{GraphQL API}'et og sendes med
hver request der kræver det (Enkelte requests er offentligt åbne og kræver ikke login).
Tokenet valideres på serveren via middleware, hvor brugerens identitet udtrækkes og tilføjes request-konteksten.
...
Dette sikrer, at tokenet ikke er direkte tilgængeligt via JavaScript, hvilket reducerer risikoen for XSS-angreb.
